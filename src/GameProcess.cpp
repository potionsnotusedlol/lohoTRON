...
void GameProcess::keyPressEvent(QKeyEvent* event) {
    ...
    const QKeySequence keys_assigned = loadKeyBind();

    switch (event->key()) {
    case keys_assigned[0]:
        m_keyForward = true;
        break;
    case keys_assigned[1]:
        m_keyBackward = true;
        break;
    case keys_assigned[2]:
        m_keyLeft = true;
        break;
    case keys_assigned[3]:
        m_keyRight = true;
        break;
    case Qt::Key_Escape:
...
QKeySequence loadKeyBind() {
    QJsonObject root = loadConfigRoot();
    QJsonObject player = root.value("player").toObject();
    const QJsonArray keys = player.value("key_bindings").toArray();
    QKeySequence res;

    for (auto &key : keys) QKeySequence(key.toString()) >> res;
